...
(function () {
	...
	//ORIGINAL: Pero esto no funciona porque a la que cambia de layout se pierde el listener
	...
	// 	}
	// });
	// Para que se mantenga el listener al cambiar de layout por lo tanto se mantiene el boton? 

	function addEventListenerOnSmHoverMenu() {
		const btn = document.getElementById("vertical-hover");
		const htmlTag = document.getElementById("html-tag-master");

		if (!btn || !htmlTag) return;

		btn.addEventListener("click", function () {
			let current = htmlTag.getAttribute("data-sidebar-size");
			let pinned = false;

			if (current === "sm-hover") {
				htmlTag.setAttribute("data-sidebar-size", "sm-hover-active");
				pinned = true;
			} else {
				htmlTag.setAttribute("data-sidebar-size", "sm-hover");
				pinned = false;
			}
			...
			fetch("/preferences", {
				...
				body: JSON.stringify({ sidebar_pinned: pinned ? 1 : 0 })
			});
		});
	}
	// set full layout
	...
	function initModeSetting() {
		...
		// Si existe el botón de cambio de tema
		if (lightDarkBtn && lightDarkBtn.length) {
			lightDarkBtn[0].addEventListener("click", function () {
				...
				// Si está en claro, cambiar a oscuro
				else {
					setLayoutMode("data-bs-theme", "dark", "layout-mode-dark", html);
					darkMode = 1;
				}
				...
				fetch("/preferences", {
					...
					// En JS el objeto hay que transformarlo a texto plano con JSON.stringify, porque el protocolo HTTP no entiende objetos JS
					body: JSON.stringify({ dark_mode: darkMode })
				});

				// Disparar resize para refrescar gráficos o elementos dependientes del tema
